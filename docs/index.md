...
hero:
  ...
  tagline: A small Rust daemon suppresses configured inputs, normalizes their edges, and routes them over a loopback-only WebSocket.
  actions:
    - theme: brand
      text: Get started
      link: /quick-start
...
features:
  ...
  - title: Managed sessions
    details: Passive broadcasts remain available, while one heartbeat-managed client can request exclusive routing.
---
...
The server binds to `127.0.0.1` only. Protocol 0.2 has no authentication, TLS, subscriptions, replay, or remote access.
