...
USAHP v0 intentionally solves one problem: deliver suppressed, normalized local switch edges to local applications.

## Protocol stability

The wire version is `0.2.0` and does not carry a compatibility promise. Clients should reject unsupported protocol versions and expect the daemon to restart with sequence numbering reset.
...
- only handshake and heartbeat client messages
...
Protocol 0.2 does not define:

- debounce or noise filtering;
- hold, repeat, chord, or gesture events;
- final confidence semantics or continuous analog samples;
- activation policy;
- multiple managed sessions or session arbitration;
